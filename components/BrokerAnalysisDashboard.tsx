...
import { STATE_ABBREVIATIONS } from '../constants'; // Import the abbreviations
...
interface BrokerAnalysisDashboardProps {
  allData: DataRecord[];
  availableBrokerNames: string[]; // Should not include "All"
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-800 p-4 sm:p-6 rounded-lg shadow-xl">
        <h2 className="text-xl font-semibold text-slate-100 mb-4">Broker Performance Analysis</h2>
        <MultiSelectDropdown
          id="broker-select"
      ...
      {selectedBrokers.length > 0 && brokerFilteredData.length > 0 && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <KpiCard title="Total Quintals Sold" value={quantityFormatter(totalQuintals)} icon={<SalesIcon />} />
            <KpiCard title="Total Order Value" value={currencyFormatter(totalOrderAmount)} icon={<AmountIcon />} />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            <ChartCard title={`Sales by Location for: ${selectedBrokers.join(', ').substring(0,50)}${selectedBrokers.join(', ').length > 50 ? '...' : ''} (Top 15)`}>
              <BarChartComponent 
            ...
            <ChartCard title={`Sales by Party for: ${selectedBrokers.join(', ').substring(0,50)}${selectedBrokers.join(', ').length > 50 ? '...' : ''} (Top 15)`}>
              <BarChartComponent 
